using EDocSys.Application.Features.IssuanceStatuses.Commands.Create;
using EDocSys.Application.Features.IssuanceStatuses.Queries.GetAllCached;
using EDocSys.Web.Areas.Documentation.Models;
using AutoMapper;
using EDocSys.Domain.Entities.Documentation;


namespace EDocSys.Web.Areas.Documentation.Mappings
{
    internal class IssuanceStatusProfile : Profile
    {
        public IssuanceStatusProfile()
        {
            CreateMap<CreateIssuanceStatusCommand, IssuanceStatusViewModel>().ReverseMap();

            CreateMap<SOP, GetAllIssuanceStatusCachedResponse>();

            CreateMap<GetAllIssuanceStatusCachedResponse, IssuanceStatusViewModel>().ReverseMap();
        }
    }
}
