...
namespace EDocSys.Web.Areas.Documentation.Controllers
{
    ...
    public class ProcedureController : BaseController<ProcedureController>
    {
        ...
        public async Task<IActionResult> Preview(int id, bool print = false, ProcedureViewModel procedure = null, int IPrint = 0, bool revert = false)
        {
            ...
            if (revert)
            {
                ...
                procedureViewModelOld.IsArchive = false;
                procedureViewModelOld.ArchiveDate = null;
                    ...
                    if (responseInfo.Succeeded)
                    {
                        ...
                        if (responseInfoH.Succeeded)
                        {
                            ...
                            if (issuanceViewModel.IssuanceStatusView == "Acknowledged")
                            {
                                ...
                                if (issuanceViewModel.DOCStatus != "New" && procedureViewModel.IsArchive == false)
                                {
        ...
        public async Task<JsonResult> OnGetAssignApprovedBy(int id)
        {
            ...
            var response = await _mediator.Send(new GetProcedureByIdQuery() { Id = id });
            var responseC2 = _context.UserApprovers.Where(a => a.ApprovalType == "APP" && (users.Select(s => s.UserCompanyId).Contains(a.CompanyId) || a.CompanyId == allCompId) && (users.Select(s => s.UserDepartmentId).Contains(a.DepartmentId) || a.DepartmentId == allDeptId)).ToList();
