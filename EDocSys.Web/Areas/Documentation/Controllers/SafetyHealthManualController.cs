...
namespace EDocSys.Web.Areas.Documentation.Controllers
{
    ...
    public class SafetyHealthManualController : BaseController<SafetyHealthManualController>
    {
        ...
        public async Task<IActionResult> Preview(int id, bool print = false, SafetyHealthManualViewModel shManual = null, bool revert = false)
        {
            ViewBag.RoleAB1 = false;
            ViewBag.RoleA = false;
            ViewBag.RoleB1 = false;
            ...
            if (revert)
            {
                var response2 = await _mediator.Send(new GetSafetyHealthManualByIdQuery() { Id = id });
                var safetyHealthManualViewModelOld = _mapper.Map<SafetyHealthManualViewModel>(response2.Data);
                safetyHealthManualViewModelOld.IsArchive = false;
                safetyHealthManualViewModelOld.ArchiveDate = null;
                var updateSafetyHealthManualCommandOld = _mapper.Map<UpdateSafetyHealthManualCommand>(safetyHealthManualViewModelOld);
                var result2 = await _mediator.Send(updateSafetyHealthManualCommandOld);
            }
            ...
            if (response.Succeeded)
            {
                ...
                var userChk = users.Select(s => s.UserCompanyId).Contains(safetyHealthManualViewModel.CompanyId);
                var validUser = false;
                if (rolesList.Contains("A"))
                {
                    ViewBag.RoleA = true;
            ...
            else
            {
                ...
                if (response.Succeeded)
                {
                    ...
                    if (rev)
                    {
                        ...
                        safetyHealthManualViewModel.PrintCount = 0;
                    }
        ...
        public async Task<JsonResult> OnGetAssignConcurred1(int id)
        {
            ...
            var responseC1 = _context.UserApprovers.Where(a => a.ApprovalType == "C1" && (users.Select(s => s.UserCompanyId).Contains(a.CompanyId) || a.CompanyId == allCompId) && (a.DepartmentId == qaId || a.DepartmentId == allDeptIdSH1)).ToList();
        ...
        public async Task<JsonResult> OnGetAssignConcurred2(int id)
        {
            ...
            var responseC2 = _context.UserApprovers.Where(a => a.ApprovalType == "C2" && (users.Select(s => s.UserCompanyId).Contains(a.CompanyId) || a.CompanyId == allCompId) && (a.DepartmentId == qaId || a.DepartmentId == allDeptIdSH2)).ToList();
        ...
        public async Task<JsonResult> OnGetAssignApprovedBy(int id)
        {
            ...
            int allCompId = 0;
            var response = await _mediator.Send(new GetSafetyHealthManualByIdQuery() { Id = id });
            var responseC2 = _context.UserApprovers.Where(a => a.ApprovalType == "APP" && (users.Select(s => s.UserCompanyId).Contains(a.CompanyId) || a.CompanyId == allCompId) && (a.DepartmentId == qaId || a.DepartmentId == allDeptIdSH2)).ToList();
