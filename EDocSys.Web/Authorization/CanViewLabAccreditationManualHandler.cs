...
{
    public class CanViewLabAccreditationManualHandler : AuthorizationHandler<CanViewLabAccreditationManualRequirement>
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly ApplicationDbContext _dbContext;
        private readonly IdentityContext _identityContext;

        public CanViewLabAccreditationManualHandler(IHttpContextAccessor httpContextAccessor, ApplicationDbContext dbContext, IdentityContext identityContext)
        {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, CanViewLabAccreditationManualRequirement requirement)
        {
            ...
            var labAccreditationManualId_GetRoute = _httpContextAccessor.HttpContext.GetRouteValue("id")?.ToString();
            int id_GetRoute;
            ...
            bool success_GetRoute = int.TryParse(labAccreditationManualId_GetRoute, out id_GetRoute);
            if (!success)
            {
            ...
            {
                if (labAccreditationManual.CompanyId == user.UserCompanyId && labAccreditationManual.Concurred1 == user.Id)
                {
                    context.Succeed(requirement);
                    return Task.CompletedTask;
                }
            ...
            {
                if (labAccreditationManual.Concurred1 == user.Id)
                {
                    context.Succeed(requirement);
                    return Task.CompletedTask;
                }
            ...
            //else
            //{
            //    if (LabAccreditationManual.DepartmentId != user.UserDepartmentId || LabAccreditationManual.CompanyId != user.UserCompanyId)
            //    {
            //        context.Fail();
            //        return Task.CompletedTask;
            //    } 
            ...
            /*
            if (LabAccreditationManual.RevisionDate == null)
            {
                if (userId != LabAccreditationManual.Concurred1 && userId != LabAccreditationManual.Concurred2 && userId != LabAccreditationManual.ApprovedBy)
                {
                    context.Fail();
                    return Task.CompletedTask;
                }
