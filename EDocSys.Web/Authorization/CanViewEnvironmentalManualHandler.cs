...
{
    public class CanViewEnvironmentalManualHandler : AuthorizationHandler<CanViewEnvironmentalManualRequirement>
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly ApplicationDbContext _dbContext;
        private readonly IdentityContext _identityContext;

        public CanViewEnvironmentalManualHandler(IHttpContextAccessor httpContextAccessor, ApplicationDbContext dbContext, IdentityContext identityContext)
        {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, CanViewEnvironmentalManualRequirement requirement)
        {
            ...
            var environmentalManualId_GetRoute = _httpContextAccessor.HttpContext.GetRouteValue("id")?.ToString();
            int id_GetRoute;
            ...
            bool success_GetRoute = int.TryParse(environmentalManualId_GetRoute, out id_GetRoute);
            if (!success)
            {
            ...
            {
                if (environmentalManual.CompanyId == user.UserCompanyId && environmentalManual.Concurred1 == user.Id)
                {
                    context.Succeed(requirement);
                    return Task.CompletedTask;
                }
            ...
            {
                if (environmentalManual.Concurred1 == user.Id)
                {
                    context.Succeed(requirement);
                    return Task.CompletedTask;
                }
            ...
            //else
            //{
            //    if (EnvironmentalManual.DepartmentId != user.UserDepartmentId || EnvironmentalManual.CompanyId != user.UserCompanyId)
            //    {
            //        context.Fail();
            //        return Task.CompletedTask;
            //    } 
            ...
            /*
            if (EnvironmentalManual.RevisionDate == null)
            {
                if (userId != EnvironmentalManual.Concurred1 && userId != EnvironmentalManual.Concurred2 && userId != EnvironmentalManual.ApprovedBy)
                {
                    context.Fail();
                    return Task.CompletedTask;
                }
