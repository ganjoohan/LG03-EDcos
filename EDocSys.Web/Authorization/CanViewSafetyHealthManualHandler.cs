...
{
    public class CanViewSafetyHealthManualHandler : AuthorizationHandler<CanViewSafetyHealthManualRequirement>
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly ApplicationDbContext _dbContext;
        private readonly IdentityContext _identityContext;

        public CanViewSafetyHealthManualHandler(IHttpContextAccessor httpContextAccessor, ApplicationDbContext dbContext, IdentityContext identityContext)
        {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, CanViewSafetyHealthManualRequirement requirement)
        {
            ...
            var safetyHealthManualId_GetRoute = _httpContextAccessor.HttpContext.GetRouteValue("id")?.ToString();
            int id_GetRoute;
            ...
            bool success_GetRoute = int.TryParse(safetyHealthManualId_GetRoute, out id_GetRoute);
            if (!success)
            {
            ...
            {
                if (safetyHealthManual.CompanyId == user.UserCompanyId && safetyHealthManual.Concurred1 == user.Id)
                {
                    context.Succeed(requirement);
                    return Task.CompletedTask;
                }
            ...
            {
                if (safetyHealthManual.Concurred1 == user.Id)
                {
                    context.Succeed(requirement);
                    return Task.CompletedTask;
                }
            ...
            //else
            //{
            //    if (SafetyHealthManual.DepartmentId != user.UserDepartmentId || SafetyHealthManual.CompanyId != user.UserCompanyId)
            //    {
            //        context.Fail();
            //        return Task.CompletedTask;
            //    } 
            ...
            /*
            if (SafetyHealthManual.RevisionDate == null)
            {
                if (userId != SafetyHealthManual.Concurred1 && userId != SafetyHealthManual.Concurred2 && userId != SafetyHealthManual.ApprovedBy)
                {
                    context.Fail();
                    return Task.CompletedTask;
                }
